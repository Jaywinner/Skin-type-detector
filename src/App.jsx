...
export default function App() {
  ...
  // Removed server-side/upload options — app runs the TFJS model in-browser only

  // --- TFJS in-browser model support ---
  useEffect(() => {
    let mounted = true
    async function loadModel() {
      ...
      try {
        ...
        // try to load metadata.json to get labels (Teachable Machine format)
        try {
  ...
  async function predictWithTfjs(file) {
    ...
    try {
      ...
      // determine target size from metadata or model input
      ...
      // build tip mapping: prefer predefined TIPS, otherwise a generic message
      ...
      <div className="container card">
        ...
        {status === 'done' && result && (
          <div className="result card result-anim">
            ...
            <p className="result-line"><strong>Recommendation:</strong> {TIPS[result.type] || 'Maintain balanced hydration and consult a professional for concerns.'}</p>
          </div>
        )}
